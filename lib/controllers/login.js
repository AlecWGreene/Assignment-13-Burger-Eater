 ...
 */
async function userLogin(arg_userName){
...
loginRouter.post("/api-login/login/", function(arg_request, arg_response, next){
    ...
    console.log("Received body: ", arg_request.body);
    ...
    verifyLogin(arg_request.body.username, arg_request.body.password).then(function(arg_id){
        arg_response.send(arg_id); 
        console.log("User " + arg_request.body.username + " has logged in");       
    });
});
...
loginRouter.post("/api-login/register/", function(arg_request, arg_response, next){
    ...
    orm.create("user_table",["user_name", "user_display", "user_hash"], [[arg_request.body.username, arg_request.body.display, t_hash]]).then(arg_response.sendStatus(202)).catch(arg_error => console.log(arg_error));
});
